...
void SocketCAN::open(std::string interface) {
  ...
  sender_thread_ = std::thread([this]() {
    can_frame tx_frame;
    while (receiving_) {
      while (tx_queue_.pop(tx_frame)) ::write(sockfd_, &tx_frame, sizeof(can_frame));
    }
  });
}
...
void SocketCAN::remove_can_callback(CanCbkId id_) {
  can_callback_list_.erase(std::remove_if(can_callback_list_.begin(), can_callback_list_.end(),
